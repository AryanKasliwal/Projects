...
// A Buffer is defined as queue with the required queue functions.
class Buffer {
private:
    int frontIdx, size;
    char **articles;
...
void* classifier(void *arg) {
    ...
    while (!QUIT || !buffer->isEmpty()) {
        ...
        // Erases all spaces and other characters from the article.
...
void* crawler(void *arg) {
    ...
    while(!QUIT) {
        ...
        if (sem_trywait(&semBuffer)) {
            ...
            printStatus("wait", id);
            sem_wait(&semBuffer);
            // The craw;er stops waiting for the buffer to be empty.
